...
fbUtils.markup = function(tag) {
	...
	var contentType = void 0, field = document.createElement(tag), getContentType = function getContentType(content) {
...
fbUtils.parseOptions = function(field) {
	var options = field.getElementsByTagName('option'), optionData = {}, data = [];
...
fbUtils.parseXML = function(xmlString) {
	...
	var xml = parser.parseFromString(xmlString, 'text/xml'), formData = [];
...
fbUtils.fieldRender = function(fieldData, opts) {
	...
	var fieldMarkup = '', fieldLabel = '', optionsMarkup = '', fieldLabelText = fieldData.label || '', fieldDesc = fieldData.description || '', fieldRequired = '', fieldOptions = fieldData.values;
	...
	if (fieldData.type !== 'hidden') {
		...
		// edit by vivid.g
		fieldLabel = '<label for="' + fieldData.id + '" class="fb-' + fieldData.type + '-label input-group-addon">' + fieldLabelText + ' ' + fieldRequired + ' ' + fieldDesc + '</label>';
	}
	...
	case 'rich-text': {
		...
		fieldMarkup = '<div class="input-group">' + fieldMarkup + '</div>';// edit by vivid.g
		break;
	}
	case 'select':
	...
	case 'radio-group': {
		...
		if (fieldOptions) {
			...
			for ( var _i = 0; _i < fieldOptions.length; _i++) {
				optionAttrs = Object.assign({
					value : '',
					label : ''
				}, fieldData, fieldOptions[_i]);
				...
				optionsMarkup += '<input ' + _optionAttrsString + ' /> <label for="' + optionAttrs.id + '">' + optionAttrs.label + '</label>';// edit by vivid.g
			}
	...
	case 'autocomplete':
		...
		if (fieldData.type === 'file') {
			fieldMarkup = fieldLabel + '<div class="form-control" style="height: auto;"><div class="suredy-file"></div></div>';
		}

		// edit by vivid.g
		if (fieldData.type === 'date') {
			fieldMarkup += '<div class="input-group-addon icon-calendar"></div>';
			fieldMarkup = fieldMarkup.replace(/type\="date"/, 'type="text"');
		}
		fieldMarkup = '<div class="input-group">' + fieldMarkup + '</div>';// edit by vivid.g
		break;
	case 'color':
	...
	case 'submit':
		...
		break;
	case 'checkbox':
		fieldMarkup = '<input ' + fieldDataString + '> ' + fieldLabel.replace(/input\-group\-addon/, '');
...
/**
 * Callback for other option. Toggles the hidden text area for "other" option.
 * 
 * @param {String} otherId id of the "other" option input
 */
fbUtils.otherOptionCB = function(otherId) {
	var otherInput = document.getElementById(otherId), otherInputValue = document.getElementById(otherId + '-value');
...
function formBuilderHelpersFn(opts, formBuilder) {
	...
	/**
	 * jQuery UI sortable beforeStop callback used for both lists. Logic for canceling the sort or drop.
	 */
	_helpers.beforeStop = function(event, ui) {
		var form = document.getElementById(opts.formID), lastIndex = form.children.length - 1, cancelArray = [];
	...
	/**
	 * hide and show mouse tracking tooltips, only used for disabled fields in the editor.
	 * 
	...
	_helpers.fieldOptionData = function(field) {
		...
		$('.sortable-options li', field).each(function() {
			var $option = $(this), selected = $('.option-selected', $option).is(':checked'), attrs = {
	...
	_helpers.xmlSave = function(form) {
		...
		var formData = _helpers.prepData(form), xml = [ '<form-template>\n\t<fields>' ];
		...
		utils.forEach(formData, function(fieldIndex, field) {
			...
			if (field.type.match(/(select|checkbox-group|radio-group)/)) {
				var optionData = field.values, options = [];
		...
		if (form.childNodes.length !== 0) {
			...
			utils.forEach(form.childNodes, function(index, field) {
				...
				if (!$field.hasClass('disabled')) {
					...
					(function() {
						var fieldData = _helpers.getTypes($field), roleVals = $('.roles-field:checked', field).map(function() {
							return this.value;
						}).get();
	...
	_helpers.incrementId = function(id) {
		var split = id.lastIndexOf('-'), newFieldNumber = parseInt(id.substring(split + 1)) + 1, baseString = id.substring(0, split);
	 ...
	 * 
	 * @param {Object} field jQuery wrapped dom object
	...
	_helpers.updatePreview = function(field) {
		...
		var fieldType = $(field).attr('type'), $prevHolder = $('.prev-holder', field), previewData = {
			type : fieldType
		}, preview;
		...
		if (previewData.type === 'file') {
			require([ 'suredyFile' ], function(File) {
				File($('.suredy-file', $prevHolder), {
					fileCount : 0,
					fileType : '',
					view : previewData.view,
					changeView : eval(previewData.changeView),
					addFile : false,
					editable : false,
					download : false,
					autoUpload : false,
		...
		// edit by vivid.g
		if (previewData.type === 'date') {
			require([ 'suredyDatetimepicker' ], function(Datetimepicker) {
				var $date = $('input[id^=date-]', $prevHolder).addClass('datetimepicker');
				Datetimepicker($date, {
					weekstart : parseInt(previewData.weekstart),
					format : (previewData.format || 'yyyy-MM-dd'),
					view : previewData.view,
					manualClose : !!eval(previewData.manualClose),
				});
			});
		}
	...
	_helpers.classNames = function(field, previewData) {
		var i = void 0, type = previewData.type, style = previewData.style;
	...
	_helpers.confirm = function(message, yesAction) {
		...
		var yes = utils.markup('button', opts.messages.yes, {
			className : 'yes btn btn-success btn-sm'
		}), no = utils.markup('button', opts.messages.no, {
		...
		var btnWrap = utils.markup('div', [ no, yes ], {
			className : 'button-wrap'
		});

		className = 'form-builder-dialog ' + className;

		var miniModal = utils.markup('div', [ message, btnWrap ], {
			className : className
		});
		if (!coords) {
			coords = {
	 ...
	 * @param {Boolean} coords false if no coords are provided. Without coordinates the popup will appear center screen.
	...
	_helpers.dialog = function(content) {
		...
		var miniModal = utils.markup('div', content, {
			className : className
		});
		if (!coords) {
			coords = {
	...
	_helpers.closeAllEdit = function(stage) {
		var fields = $('> li.editing', stage), toggleBtns = $('.toggle-form', stage), editModes = $('.frm-holder', fields);
	...
	_helpers.toggleEdit = function(fieldId) {
		var field = document.getElementById(fieldId), toggleBtn = $('.toggle-form', field), editMode = $('.frm-holder', field);
	...
	_helpers.stickyControls = function($sortableFields, cbUL) {
		...
		var $cbWrap = $(cbUL).parent(), $stageWrap = $sortableFields.parent(), cbWidth = $cbWrap.width(), cbPosition = cbUL.getBoundingClientRect();
		...
		$(window).scroll(function() {
			...
			if (scrollTop > $stageWrap.offset().top) {
				...
				var cbOffset = $cbWrap.offset(), stageOffset = $stageWrap.offset(), cbBottom = cbOffset.top + $cbWrap.height(), stageBottom = stageOffset.top + $stageWrap.height();
	...
	_helpers.showData = function() {
		var data = utils.escapeHtml(formBuilder.formData), code = utils.markup('code', data, {
			className : 'formData-' + opts.dataType
	...
	_helpers.removeField = function(fieldID) {
		var fieldRemoved = false, form = document.getElementById(opts.formID), fields = form.getElementsByClassName('form-field');
		...
		var field = document.getElementById(fieldID), $field = $(field);
		if (!field) {
...
(function($) {
	...
	var Toggle = function Toggle(element, options) {
		...
		var opts = $.extend(defaults, options), $kcToggle = $('<div class="kc-toggle"/>').insertAfter(element).append(element);

		$kcToggle.toggleClass('on', element.is(':checked'));

		var kctOn = '<div class="kct-on">' + opts.messages.on + '</div>', kctOff = '<div class="kct-off">' + opts.messages.off + '</div>', kctHandle = '<div class="kct-handle"></div>', kctInner = '<div class="kct-inner">' + kctOn + kctHandle + kctOff + '</div>';
...
(function($) {
	var FormBuilder = function FormBuilder(options, element) {
		...
		defaults.messages.subtypes = function() {
			...
			return {
				text : [ 'text', 'password' ].map(subtypeDefault),// edit by vivid.g
		...
		var opts = Object.assign({}, defaults, options), frmbID = 'frmb-' + $('ul[id^=frmb-]').length++;
		...
		var lastID = frmbID + '-fld-1', boxID = frmbID + '-control-box';
		...
		if (opts.showActionButtons) {
			...
			var viewDataText = opts.dataType === 'xml' ? opts.messages.viewXML : opts.messages.viewJSON, viewData = utils.markup('button', viewDataText, {
				id : frmbID + '-view-data',
				type : 'button',
				...
				className : 'form-actions btn-group pull-right'// edit by vivid.g
			});
		...
		var nonEditableFields = function nonEditableFields() {
			...
			if (opts.prepend && !$('.disabled.prepend', $sortableFields).length) {
				var prependedField = utils.markup('li', opts.prepend, {
			...
			if (opts.append && !$('.disabled.append', $sortableFields).length) {
				var appendedField = utils.markup('li', opts.append, {
		...
		var fieldOptions = function fieldOptions(values) {
			var optionActions = [ utils.markup('a', opts.messages.addOption, {
				className : 'add add-opt'
			}) ], fieldOptions = [ '<label class="false-label">' + opts.messages.selectOptions + '</label>' ], isMultiple = values.multiple || values.type === 'checkbox-group';
		...
		var advFields = function advFields(values) {
			var advFields = [], key, optionFields = [ 'select', 'checkbox-group', 'radio-group' ], isOptionField = function() {
				return optionFields.indexOf(values.type) !== -1;
			}(), valueField = !utils.inArray(values.type, [ 'header', 'paragraph', 'file' ].concat(optionFields)), roles = values.role !== undefined ? values.role.split(',') : [];

			advFields.push(requiredField(values));

			if (values.type === 'checkbox') {
				advFields.push(boolAttribute('toggle', values, {
					first : opts.messages.toggle
				}));
			}
			...
			for (key in opts.roles) {
				if (opts.roles.hasOwnProperty(key)) {
					var checked = utils.inArray(key, roles) ? 'checked' : '', roleId = 'fld-' + lastID + '-roles-' + key;
		...
		function inputUserAttrs(name, attrs) {
			...
			var textInput = '<input ' + utils.attrString(textAttrs) + '>', inputWrap = '<div class="input-wrap">' + textInput + '</div>';
		...
		function selectUserAttrs(name, options) {
			...
			var select = '<select ' + utils.attrString(selectAttrs) + '>' + optis.join('') + '</select>', inputWrap = '<div class="input-wrap">' + select + '</div>';
			return '<div class="form-group ' + name + '-wrap">' + label + inputWrap + '</div>';
		}
		...
		var boolAttribute = function boolAttribute(name, values, labels) {
			...
			}, checked = values[name] !== undefined ? 'checked' : '', input = '<input type="checkbox" class="fld-' + name + '" name="' + name + '" value="true" ' + checked + ' id="' + name + '-' + lastID + '"/> ', left = [], right = [ input ];
		...
		var btnStyles = function btnStyles(style, type) {
			var tags = {
				button : 'btn'
			}, styles = opts.messages.styles[tags[type]], styleField = '';
		...
		var numberAttribute = function numberAttribute(attribute, values) {
			...
			var attrVal = values[attribute], attrLabel = opts.messages[attribute] || attribute, placeholder = opts.messages.placeholders[attribute], inputConfig = {
				type : 'number',
				value : attrVal,
				name : attribute,
				min : '0',
				...
				id : attribute + '-' + lastID
			}, numberAttribute = '<input ' + utils.attrString(utils.trimObj(inputConfig)) + '>', inputWrap = '<div class="input-wrap">' + numberAttribute + '</div>';
		...
		var selectAttribute = function selectAttribute(attribute, values, optionData) {
			...
			var select = '<select ' + utils.attrString(selectAttrs) + '>' + selectOptions.join('') + '</select>', inputWrap = '<div class="input-wrap">' + select + '</div>';
		...
		var textAttribute = function textAttribute(attribute, values) {
			...
			var attrVal = values[attribute] || '', attrLabel = opts.messages[attribute];
			...
			var placeholders = opts.messages.placeholders, placeholder = placeholders[attribute] || '', attributefield = '', noMakeAttr = [];
		...
		var requiredField = function requiredField(values) {
			var noRequire = [ 'header', 'paragraph', 'button' ], noMake = [], requireField = '';
			...
			if (!noMake.some(function(elem) {
				...
				requireField = boolAttribute('required', values, {
					first : opts.messages.required
				});
			}
		...
		var appendNewField = function appendNewField(values) {
			var type = values.type || 'text', label = values.label || opts.messages[type] || opts.messages.label, delBtn = utils.markup('a', opts.messages.remove, {
				id : 'del_' + lastID,
			...
			liContents += utils.markup('a', opts.messages.close, {
			...
			var field = utils.markup('li', liContents, {
				'class' : type + '-field form-field ' + (values.colWidth),// edit by vivid.g
				'type' : type,
				id : lastID
			}), $li = $(field);
		...
		var selectFieldOptions = function selectFieldOptions(name, optionData, multipleSelect) {
			...
			}, optionDataOrder = [ 'value', 'label', 'selected' ], optionInputs = [];
		...
		var cloneItem = function cloneItem(currentItem) {
			var currentId = currentItem.attr('id'), type = currentItem.attr('type'), ts = new Date().getTime(), cloneName = type + '-' + ts;
		...
		$sortableFields.on('click touchstart', '.toggle-form, .close-field', function(e) {
			...
			if (e.handled !== true) {
				...
				_helpers.closeAllEdit($sortableFields);// edit by vivid.g
				_helpers.toggleEdit(targetID);// edit by vivid.g
				e.handled = true;
			} else {
				return false;
			}
		...
		// edit by vivid.g
		$sortableFields.on('keyup change', '[name="colWidth"]', function() {
			$(this).closest('li').removeClass('full').removeClass('half').removeClass('small').removeClass('big-small').addClass($(this).val());
		});
		...
		$sortableFields.on('click touchstart', '.icon-copy', function(e) {
			...
			_helpers.save();
			_helpers.closeAllEdit($sortableFields);// edit by vivid.g
			_helpers.toggleEdit($clone.attr('id'));// edit by vivid.g
		});
		...
		$sortableFields.on('click touchstart', '.delete-confirm', function(e) {
			...
			var buttonPosition = this.getBoundingClientRect(), bodyRect = document.body.getBoundingClientRect(), coords = {
				pageX : buttonPosition.left + buttonPosition.width / 2,
				pageY : buttonPosition.top - bodyRect.top - 12
			};

			var deleteID = $(this).parents('.form-field:eq(0)').attr('id'), $field = $(document.getElementById(deleteID));
			...
			if (opts.fieldRemoveWarn) {
				var warnH3 = utils.markup('h3', opts.messages.warning), warnMessage = utils.markup('p', opts.messages.fieldRemoveWarning);
		...
		$sortableFields.on('click', '.style-wrap button', function() {
			var styleVal = $(this).val(), $parent = $(this).parent(), $btnStyle = $parent.prev('.btn-style');
		...
		$sortableFields.on('click', 'input.fld-access', function() {
			var roles = $(this).parents('li.form-field').find('div.available-roles'), enableRolesCB = $(this);
		...
		$sortableFields.on('click', '.add-opt', function(e) {
			e.preventDefault();
			var $optionWrap = $(this).parents('.field-options:eq(0)'), $multiple = $('[name="multiple"]', $optionWrap), $firstOption = $('.option-selected:eq(0)', $optionWrap), isMultiple = false;
