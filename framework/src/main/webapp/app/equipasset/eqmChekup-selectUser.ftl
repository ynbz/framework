...
<script type="text/javascript">
	require([ 'suredyTree', 'suredyList', 'suredyModal','validation' ], function(Tree, List, Modal) {
		Tree('.unit-user-tree', '${request.contextPath}/mobile/eqm-checkup/ou/tree/true/false/true', {
			style : 'department'
		});
		...
		$('.unit-user-tree').on(Tree.nodeClick, function(event, $node) {
			...
			if(nodeisActive){
				...
				$.post('${request.contextPath}/mobile/eqm-checkup-update/getSelectUser.do',param,function(html, textStatus, jqXHR) {
					var $html = $(html);
		...
		var listConfig = ({
			...
			paginate : function(page,size,key) {
				...
				var unit = $('#unit').val();
				var falg = $('#falg').val();
				var url='${request.contextPath}/mobile/eqm-checkup-update/getSelectUser.do';
