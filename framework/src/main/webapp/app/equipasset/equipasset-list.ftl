	...
	<div class="col-md-9" valign="top" style="padding-left: 5px;">
			...
			<div class="row" style="padding-top: 10px;">
				<div class="form-inline">
					...
					<div class="btn btn-info" id="upadatebtn">
						<i class="icon-edit"></i> 修改
					</div>
					<div class="btn btn-danger" id="deletebtn">
						<i class="icon-remove"></i> 删除
					</div>	
					<div class="btn btn-danger" id="downloadEquipAsset">
						<i class="icon-signout"></i> 导出设备
					</div>			
...
require(['suredyList','suredyTree','jqueryForm','suredyModal','suredyTreeSelector'], function(List, Tree,UpLoad,Modal,TreeSelector) {
	...
	tempList=List;
	 var ouTree = function() {
		...
		$('.type-tree').on(Suredy.Tree.nodeClick, function(event, $node) {			
			...
			var url='${request.contextPath}/EquipAssetCtrl/getEquipData.do';
	...
	listConfig = ({
		...
		paginate : function(page, size, key, sort) {
			...
			var url='${request.contextPath}/EquipAssetCtrl/getEquipData.do?page=' + page + '&size=' + size+'&typeId='+typeId;
		...
		$("#properSearchbtn").click(function(){
			var url='${request.contextPath}/EquipAssetCtrl/getEquipData.do?typeId='+$('#nodeId').val();
			$.post(url, $('#form').serialize() , function(html, textStatus, jqXHR) {
				var $html = $(html);
				$("#equipmentList").html($("#equipmentList", $html).html());
				List('.equipment-list',listConfig);
		...
		$("#downloadEquipAsset").click(function(){ 
			var form = $('#form');
			
			//form.css('top', '-1000px').css('position', 'fixed');
			
			//form.appendTo($('body'));
			
			var url='${request.contextPath}/EquipAssetCtrl/downloadEquipAsset.do?typeId='+$('#nodeId').val();
			...
			//window.location='${request.contextPath}/EquipAssetCtrl/downloadEquipAsset.do?typeId='+$('#nodeId').val()+'&equipasset='+$('#form').serialize();
		});
		...
		$("#deletebtn").click(function(){
			...
			if (checked.length == 0 ) {
				...
				alert('只能删除一个物品!');
				return;
			}else if ($(checked[0]).find('td').eq(6).attr('id')!=1) {
				if($(checked[0]).find('td').eq(6).attr('id')==2||$(checked[0]).find('td').eq(6).attr('id')==3){
					alert('由于设备在申请中，该设备已经锁定，只能查看不能操作！');
				}else{
					alert('只能删除待分配设备!');
				}	
				return;
				...
				//	checked.eq(i).find('td').eq(1).html();
					chechedids=chechedids+","+checked.eq(0).find('td').eq(1).html();
				//}	
				var prame={
						typeId:$('#nodeId').val()
				}
				var url='${request.contextPath}/EquipAssetCtrl/deletedata.do';
				$.ajax({
					type:'post',
		...
		$("#adddatabtn").click(function(){
			...
			window.open('${request.contextPath}/EquipAssetCtrl/getSaveAndUpdatePage.do?nodeid='+nodeid)
		});
		$("#upadatebtn").click(function(){
			var checked = List.checked($('.equipment-list'));
			...
			if (checked.length == 0 ) {
				...
				alert('只能修改一个物品!');
				return;
			}else if ($(checked[0]).find('td').eq(6).attr('id')==0) {
				alert('已报废设备不能再操作!');
				return;
			}else if ($(checked[0]).find('td').eq(6).attr('id')==2||$(checked[0]).find('td').eq(6).attr('id')==3) {
				alert('由于设备在申请中，该设备已经锁定，只能查看不能操作！');
				return;
			} else {
				var dataId = $(checked[0]).find('td').eq(1).html();
				window.open('${request.contextPath}/EquipAssetCtrl/getSaveAndUpdatePage.do?equipid='+dataId+'&nodeid='+	$('#nodeId').val());
			}
		});
		/* 
