	...
	<div class="col-md-12" style="margin-left: 5px;">
		<div class="row" style="padding-top: 10px;">
			<div class="form-inline">
				<div class="btn btn-info" id="newPro">
					<i class="icon-plus"></i> 创建生产计划
				</div>
		...
		<div class="row" style="overflow-x: auto;padding-top: 5px;">
			<div id="proList">
				<table class="pro-list" data-page="<#if pageIndex??>${pageIndex}<#else>1</#if>" data-page-size="<#if pageSize??>${pageSize}<#else>20</#if>" data-count="<#if count??>${count}<#else>0</#if>"style="width: 100%;">
					<tr class="title-row">
					...
					<tr  style="cursor: pointer;" data-uri="${request.contextPath}/project/view/${pro.id}">
						<td style="display: none" >${dism.id}</td>
						<td>${pro.title}</td>
						<td>${pro.creatorFullName}</td>
						<td>${(pro.createTime?string('yyyy-MM-dd hh:MM:ss'))!''}</td>
					</tr>
					</#list> </#if>
				</table>
			</div>
...
require(['suredyList','jqueryForm','suredyDatetimepicker'], function(List) {
	var listConfig = ({
		...
		paginate : function(page, size) {
			var url='${request.contextPath}/project/list.do?page=' + page + '&size=' + size;
			$.post(url, $('#form').serialize() , function(html, textStatus, jqXHR) {
				var $html = $(html);
					$("#proList").html($("#proList", $html).html());
					List('.pro-list',listConfig);
			}, 'html');
		}
	});
    List('.pro-list',listConfig);	
	...
	$("#properSearchbtn").click(function(){
		var url='${request.contextPath}/project/list.do';
		$.post(url, $('#form').serialize() , function(html, textStatus, jqXHR) {
			var $html = $(html);
			$("#proList").html($("#proList", $html).html());
			List('.pro-list',listConfig);
	...
	$("#newPro").click(function(){
		window.open('${request.contextPath}/project/newpro/${fileTypeCode}');
	});
});
...
<script type="text/javascript">
	$(function() {
		$('.pro-list').delegate('tr', 'click', function(event) {
			var $this = $(this);
