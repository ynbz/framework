...
(function($) {
	...
	var defaults = {
		...
		//   {success:true, data: [....]} or for error {success:false, error:'Something terrible happened'}
		...
		// or absolute
		// http://localhost/schedule/templates/
		tmpl_path: 'templates/',
		tmpl_cache: true,
		classes: {
			months: {
	...
	var strings = {		
		...
		error_where:      'Calendar: 导航指示错误 {0}. 只能是 "下一个" 或 "上一个" 或 "今天"',
	...
	Calendar.prototype._calculate_hour_minutes = function(data) {
		...
		$.each(data.events, function(k, e) {
			...
			if(e.start < start.getTime()) {
				warn(1);
				e.start_hour = $self.locale['ms' + f.getMonth()] + '' + f.getDate() + '日 ' + e.start_hour;
			}

			if(e.end > end.getTime()) {
				warn(1);
				e.end_hour =  $self.locale['ms' + f.getMonth()] + '' + f.getDate() + '日 ' + e.end_hour;
			}
			...
			case 'string':
				if(source.length) {
					loader = function() {
						...
						var params = {startTime: d_from.getTime(), endTime: d_to.getTime()};
