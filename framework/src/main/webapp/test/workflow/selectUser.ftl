	...
	require([ 'suredyTree', 'suredyList', 'suredyModal','validation' ], function(Tree, List, Modal) {
		...
		$('.unit-user-tree').on(Tree.nodeClick, function(event, $node) {
			...
			if(nodeisActive){
				...
				$.post('${request.contextPath}/workflow/getSelectUser.do',param,function(html, textStatus, jqXHR) {
					var $html = $(html);
		...
		var listConfig = ({
			...
			paginate : function(page,size,key) {
				...
				var unit = $('#unit').val();
				var falg = $('#falg').val();
				var url='${request.contextPath}/workflow/getSelectUser.do';
	 ...
	 $("#btn-confirm").click(function(){
		 var executeNameId = $('#executeNameId');
		 var executeName = $('#executeName');
		 ...
		 if (checked.length == 0 ) {
			...
			return;
			 ...
			 var name = checked.find('td').eq(3).html();
			 
			 var chk_value; 
			 $('input[name="checkAll"]:checked').each(function(){ 
			 	chk_value=$(this).val(); 
			 });
			 executeNameId.val(id);
			 executeName.val(name);
			 $('#form').data('bootstrapValidator').updateStatus('executeName','NOT_VALIDATED',null).validateField('executeName');
